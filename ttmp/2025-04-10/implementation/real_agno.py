...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("htn_agent.log")],
)
...
class AgnoAgentWrapper:
    ...
    async def arun(self, prompt: str) -> Any:
        ...
        while retries <= self.max_retries:
            try:
                response = await self.agent.aprint_response(prompt)
                end_time = time.time()
                ...
                logger.warning(
                    f"Error calling agent (attempt {retries}/{self.max_retries}): {str(e)}"
                )
            ...
            for i, json_str in enumerate(matches):
                try:
                    ...
                    parse_metadata = {
                        ...
                        "error": str(e),
                        "sample": json_str[:200]
                        + ("..." if len(json_str) > 200 else ""),
                    }
                    save_trace("json_parsing_attempt", None, parse_metadata)

                    logger.debug(
                        f"Failed to parse JSON from code block, trying next match"
                    )
                    continue
        ...
        if matches:
            ...
            for i, json_str in enumerate(matches):
                try:
                    ...
                    logger.debug(
                        f"Failed to parse JSON from object pattern, trying next match"
                    )
                    continue
...
def create_agno_agent(
    model_name: str, api_key: str, instructions: str = None, tools: list = None
):
    """
