...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("htn_agent_demo.log")],
)
...
async def run_demo(
    ...
    # Log run start
    logger.info(
        f"Starting HTN Planning Agent demo with {model_name} (Run ID: {run_id})"
    )
    ...
    try:
        ...
        agent = create_agno_agent(
            ...
            tools=[
                ...
                # SerpApiTools(),
                DuckDuckGoTools()
            ],  # Add any tools you want to include
        )
        ...
        logger.info(
            f"Plan Adaptation Frequency: Every {planner.adaptation_frequency} tasks"
        )
        logger.info(f"\n===== Planning & Execution =====\n")
        ...
        if save_trace:
            execution_metrics = {
                ...
                "tasks_total": (
                    len(planner.task_manager.tasks)
                    if hasattr(planner, "task_manager")
                    else 0
                ),
                "tasks_completed": (
                    sum(
                        1
                        for task in planner.task_manager.tasks.values()
        ...
        if planner.state and planner.state.facts:
            ...
            if save_trace:
                save_trace(
                    "demo_facts_gathered", planner.state.facts, {"run_id": run_id}
                )
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run HTN Planning Agent demo with real Agno agent"
    )
    parser.add_argument(
        "--topic", type=str, default="Quantum Computing", help="Research topic"
    )
    parser.add_argument(
        "--goal",
        type=str,
        default="explains recent developments and future prospects",
        help="Research goal",
    )
    parser.add_argument(
        "--steps", type=int, default=10, help="Maximum number of execution steps"
    )
    parser.add_argument(
        "--model", type=str, default="claude-3-sonnet-20240229", help="LLM model to use"
    )
    parser.add_argument(
        "--api-key-env",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="How often to adapt the plan",
    )
